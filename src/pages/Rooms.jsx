...
function Rooms() {
	...
	const handleSubmit = async (e) => {
		...
		try {
			const response = await fetch(`${api.flask_api}/rooms/new`, {
				method: 'POST',
				headers: {
		...
		<div className='container mx-auto px-4 py-8 flex'>
			...
			<div className='w-1/2 pl-8'>
				...
				<form onSubmit={handleSubmit} className='space-y-4'>
					...
					<button
						type='submit'
						className='btn btn-ghost bg-primary text-white px-4 py-2 rounded-md hover:bg-blue-600 focus:outline-none focus:bg-blue-600 w-full'>
						Create Room
					</button>
				</form>
			</div>
